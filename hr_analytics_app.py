...
def verify_and_correct_response(response, consultant_type="legal"):
    ...
    # Check Article 77 mentions conditional - but NOT if response says it doesn't apply
    if 'المادة 77' in response or 'مادة 77' in response:
        if 'لا تنطبق' not in response and 'لا تُطبق' not in response and 'لا علاقة' not in response:
            if 'ما لم يتضمن العقد' not in response and 'تضمّن العقد' not in response and 'تضمن العقد' not in response and 'يُعمل بالعقد' not in response:
                response += '\n\n**ملاحظة بخصوص المادة 77:** التعويضات تُطبق فقط إذا لم يتضمن العقد شرط تعويض محدد. إذا تضمّن العقد تعويضاً محدداً فيُعمل بالعقد.'
...
def search_kb(question, kb_dict, max_results=3):
    """Search knowledge base by attention to words in the question.
    Returns relevant KB entries as reference text (NOT as forced answer)."""
    q = question.lower().strip()
    if not q: return ""
    ...
    for pattern_str, answer in kb_dict.items():
        ...
        # Score: count how many KB keywords appear in the question
        score = 0
        for kw in keywords:
    ...
    refs = [entry for _, entry in scored[:max_results]]
    return "\n\n---\n\n".join(refs)
            ...
            **ملاحظة:** هذا المستشار متخصص في المفاهيم العلمية والأطر المنهجية لإدارة الموارد البشرية. للاستشارات القانونية ونظام العمل، استخدم **المستشار القانوني**.
            ...
            if '_pending_hr_q' in st.session_state:
                ...
                with st.spinner("جاري التحليل المهني..."):
                    try:
                        # Search KB + RAG for relevant reference
                        kb_ref = search_kb(pending_q, HR_KB, max_results=2)
                        rag_ctx = ""
                        try:
                            if '_knowledge_engine' in st.session_state:
