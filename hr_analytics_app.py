...
class ModelOrchestrator:
    ...
    def __init__(self):
        ...
        self._instant_labor = {
            ...
            "ما هي نسبة اشتراكات التأمينات الاجتماعية؟": "**نسب الاشتراك في التأمينات الاجتماعية (GOSI):**\n\n**السعوديون:**\n- المعاشات: 9% (الموظف) + 9% (صاحب العمل)\n- الأخطار المهنية: 2% (صاحب العمل)\n- ساند (التعطل): 0.75% + 0.75%\n- **إجمالي خصم الموظف: 9.75%** (معاشات 9% + ساند 0.75%)\n- **إجمالي على الشركة: 11.75%** (معاشات 9% + ساند 0.75% + أخطار 2%)\n\n**غير السعوديين:**\n- الأخطار المهنية فقط: 2% (صاحب العمل)\n- لا يوجد خصم على الموظف",
            "ما هي فترة التجربة وشروطها؟": "**المادة 53 (معدلة بمرسوم م/44 لعام 1446هـ):**\n\n- **المدة القصوى:** 180 يوماً كحد أقصى من بداية العقد\n- **التحديد:** يجب النص عليها صراحة في العقد\n- **التمديد:** محظور بعد التعديل\n- **لا تشمل:** إجازة عيد الفطر والأضحى والإجازات المرضية\n- **حق الإنهاء:** لكلا الطرفين بدون تعويض أو إشعار\n- **لا تتكرر:** إلا في مهنة مختلفة أو بعد 6 أشهر",
            "ما هي حقوق المرأة العاملة في نظام العمل؟": "**حقوق المرأة في نظام العمل السعودي (معدل 1446هـ):**\n\n- **إجازة وضع:** 12 أسبوعاً بأجر كامل: 6 إلزامية بعد الولادة + 6 حسب الرغبة (المادة 151 معدلة)\n- **طفل مريض/ذوي احتياجات:** شهر إضافي بأجر + شهر بدون أجر\n- **ساعة رضاعة:** ساعة يومياً لمدة 24 شهراً\n- **حماية من الفصل:** أثناء الحمل وإجازة الوضع و180 يوماً بعد الوضع\n- **إجازة وفاة زوج:** 4 أشهر و10 أيام (عدة)\n- **المساواة:** أجر متساوٍ للعمل المتساوي\n- **مكافأة كاملة:** إذا أنهت العقد خلال 6 أشهر من الزواج أو 3 أشهر من الوضع",
        }
        self._instant_hr = {
...
class ReasoningLayer:
    ...
    LABOR_TOPICS = {
        ...
        'worker_rights_abuse': {
            'keywords': ['تعدي','اعتداء','ضرب','إيذاء','إهانة','تحرش','سب','شتم','ظلم',
                         'abuse','assault','harassment','بيئة عمل سامة','تنمر','عنف'],
            'articles': ['80','81','65','66'],
            'category': 'التعدي والإساءة في بيئة العمل',
            'priority': 'high'
        },
        'worker_leave_without_notice': {
            'keywords': ['ترك بدون إشعار','أترك العمل','ترك فوري','إخلال صاحب العمل','غش في العقد'],
            'articles': ['81'],
    ...
    PARTY_SIGNALS = {
        'employee': {
            'ar': ['حقوقي','تم فصلي','فُصلت','تم طردي','مستحقاتي','راتبي','إجازتي',
                   ...
                   'تم التعدي علي','تم الاعتداء','تم ضربي','تعرضت لل','تم إهانتي',
                   'تم التحرش','تم تهديدي','تم سبي','تم ظلمي'],
            'en': ['my rights','i was fired','i was terminated','my salary','my contract',
                   ...
                   'i was assaulted','i was harassed','i was abused'],
        },
        'employer': {
    ...
    def analyze_question(self, question, advisor_type, asking_party=None):
        ...
        case_keywords = ['حالتي','تم فصلي','تم طردي','حالة','situation','case','ظرفي','مشكلتي',
                         'تم التعدي','تم الاعتداء','تم ضربي','تم إهانتي','تعرضت','تم التحرش','تم تهديدي']
        ...
        # Vague query detection — short queries with insufficient context
        # Flag for clarification instead of letting the LLM guess and hallucinate
        word_count = len(question.split())
        analysis['is_vague'] = False
        analysis['clarification_needed'] = []
        if word_count <= 5:
            if not analysis['matched_articles'] and not analysis.get('topics'):
                analysis['is_vague'] = True
                analysis['clarification_needed'].append("السؤال عام جداً - يجب طلب تفاصيل إضافية قبل الإجابة")
            elif is_case and not has_entities:
                # Case analysis without details (e.g. "تم التعدي علي" - who attacked? type?)
                analysis['is_vague'] = True
                analysis['clarification_needed'].append("حالة عملية بدون تفاصيل كافية - يجب السؤال عن التفاصيل")
    ...
    def _apply_labor_rules(self, analysis):
        ...
        # Vague query handling — MUST ask for clarification instead of guessing
        if analysis.get('is_vague'):
            clarifications = analysis.get('clarification_needed', [])
            rules.append("⚠️ تحذير: السؤال غامض أو ناقص التفاصيل. يجب عليك:")
            rules.append("1. اذكر ما فهمته من السؤال بشكل مختصر")
            rules.append("2. اطلب من السائل توضيح التفاصيل المفقودة (من المعتدي؟ نوع العقد؟ مدة الخدمة؟)")
            rules.append("3. قدم إجابة أولية عامة مع ذكر جميع الاحتمالات القانونية الممكنة")
            rules.append("4. لا تفترض تفاصيل لم يذكرها السائل — الافتراض يؤدي لإجابة خاطئة")
        ...
        rules.append("لا تختر مادة قانونية بناءً على كلمة واحدة مشتركة — افهم سياق المادة كاملاً قبل الاستشهاد بها")
        ...
        if 'worker_rights_abuse' in topics:
            rules.append("تنبيه حاسم - فرّق بدقة بين المادة 80 والمادة 81:")
            rules.append("المادة 80: حق صاحب العمل في فسخ العقد إذا اعتدى العامل على صاحب العمل")
            rules.append("المادة 81: حق العامل في ترك العمل بدون إشعار إذا وقع عليه اعتداء من صاحب العمل")
            rules.append("إذا قال السائل 'تم التعدي علي' أو 'تم الاعتداء علي' = هو الضحية → استخدم المادة 81 وليس المادة 80")
            rules.append("إذا لم يتضح من المعتدي، اسأل السائل: هل التعدي من صاحب العمل أم من زميل؟")
        if 'worker_leave_without_notice' in topics:
            rules.append("المادة 81 تنطبق: يحق للعامل ترك العمل فوراً بدون إشعار في حالات محددة")
        ...
        if 'leave' in topics and 'women_rights' in topics:
            rules.append("وضّح إجازة الوضع (12 أسبوع بعد تعديل 1446هـ) وساعة الرضاعة وحماية الفصل")
    ...
    def validate_generated_answer(self, answer, analysis, advisor_type, grounded_articles=None):
        ...
        factual_errors = [
            ...
            # Wrong probation period (outdated info about extension or 90-day limit)
            (r'(?:فترة التجربة|تجربة).*(?:90 يوم|ثلاثة أشهر).*(?:تمديد|تمدد)', "خطأ محتمل: بعد تعديل 1446هـ فترة التجربة 180 يوم كحد أقصى من البداية بدون تمديد"),
            # Wrong GOSI rates - employee total is 9.75% (pension 9% + SANED 0.75%)
            (r'(?:خصم|اشتراك)\s*(?:الموظف|العامل)\s*(?:السعودي)?\s*(?::|=|هو|يبلغ)\s*(?:10\.5|12\.5)', "خطأ محتمل: خصم الموظف السعودي 9.75% (معاشات 9% + ساند 0.75%) وتحمل الشركة 11.75% (معاشات 9% + ساند 0.75% + أخطار 2%)"),
        ]

        # === ART 80/81 CONFUSION CHECK ===
        # Critical: Detect when LLM cites Art 80 (employer's right) for an assault victim (should be Art 81)
        asking_party = analysis.get('asking_party', 'neutral')
        question_lower = analysis.get('original_question', '').lower()
        victim_signals = ['تم التعدي', 'تم الاعتداء', 'تعرضت', 'تم ضربي', 'تم إهانتي',
                          'تم التحرش', 'اعتدى علي', 'ضربني', 'أهانني', 'تم تهديدي']
        is_assault_victim = any(sig in question_lower for sig in victim_signals) or asking_party == 'employee'

        if is_assault_victim:
            # Check if answer primarily cites Art 80 (wrong for victims) without Art 81
            cites_80 = bool(re.search(r'(?:المادة|مادة)\s*80', answer))
            cites_81 = bool(re.search(r'(?:المادة|مادة)\s*81', answer))
            # If any victim signal in question and answer cites 80 without 81 = likely confusion
            if any(sig in question_lower for sig in victim_signals):
                if cites_80 and not cites_81:
                    issues.append("خطأ: تم ذكر المادة 80 (حق صاحب العمل) بينما السائل ضحية تعدي — المادة الصحيحة هي 81 (حق العامل في ترك العمل)")
                elif cites_80 and cites_81:
                    # Both cited - OK but warn to emphasize 81
                    warnings.append("تأكد من التركيز على المادة 81 (حق العامل) لأن السائل هو الضحية")
...
LABOR_KB = {
    ...
    "تأمينات|تأمين اجتماعي|gosi|اشتراك|خصم|نسبة الاشتراك|ساند|أخطار مهنية": "**نظام التأمينات الاجتماعية (GOSI):**\n\n**فرع المعاشات (سعوديون فقط):**\n- الموظف: 9%\n- صاحب العمل: 9%\n\n**فرع الأخطار المهنية (الجميع):**\n- صاحب العمل: 2%\n\n**ساند - التعطل عن العمل (سعوديون):**\n- الموظف: 0.75%\n- صاحب العمل: 0.75%\n\n**الإجمالي:**\n- **خصم الموظف السعودي: 9.75%** (معاشات 9% + ساند 0.75%)\n- **تحمّل الشركة للسعودي: 11.75%** (معاشات 9% + ساند 0.75% + أخطار مهنية 2%)\n- **تحمّل الشركة لغير السعودي: 2% فقط** (أخطار مهنية)\n\n**الأجر الخاضع:** الراتب الأساسي + بدل السكن\n**الحد الأقصى للأجر الخاضع:** 45,000 ريال\n\n**ساند (التعطل):** يصرف 60% من الأجر لمدة 3 أشهر ثم 50% لـ 9 أشهر (بحد أقصى 9,000 ريال).",
    ...
    "فترة التجربة|تجربة|probation": "**فترة التجربة (المادة 53 - معدلة 1446هـ):**\n- 180 يوماً كحد أقصى من البداية (تُحدد صراحة في العقد)\n- التمديد محظور بعد التعديل\n- لا تشمل الأعياد والإجازات المرضية\n- لكلا الطرفين حق الإنهاء خلالها بدون تعويض أو إشعار\n- لا تتكرر لدى نفس صاحب العمل إلا في مهنة مختلفة أو بعد 6 أشهر",
    "حقوق المرأة|المرأة|امرأة|حامل|وضع|رضاعة|أمومة|maternity": "**حقوق المرأة العاملة (معدل 1446هـ):**\n- إجازة وضع: 12 أسبوعاً بأجر كامل (المادة 151 معدلة): 6 إلزامية بعد الولادة + 6 حسب الرغبة\n- طفل مريض/ذوي احتياجات: شهر بأجر + شهر بدون أجر\n- رضاعة: ساعة يومياً لـ 24 شهراً\n- حماية من الفصل أثناء الحمل والوضع و180 يوماً بعد الوضع\n- إجازة وفاة الزوج: 4 أشهر و10 أيام\n- مكافأة كاملة إذا أنهت خلال 6 أشهر من الزواج أو 3 أشهر من الوضع",
...
def main():
    ...
    # =========================================
        ...
        ibox("المادة 107 (معدلة): أجر ساعة الإضافي = أجر الساعة + 50%. يمكن التعويض بإجازات بموافقة الطرفين.")
...
**المادة 53 (معدلة 1446هـ):** فترة التجربة لا تزيد عن 180 يوماً كحد أقصى من البداية، تُحدد صراحة في العقد. لا يجوز تمديدها بعد التعديل. لا تدخل فيها إجازة العيدين والإجازة المرضية. لكلا الطرفين حق الإنهاء خلالها.
...
**المادة 74:** ينتهي عقد العمل بالحالات: اتفاق الطرفين كتابة، انتهاء المدة، إرادة أحد الطرفين (غير محدد)، القوة القاهرة، إغلاق المنشأة نهائياً، إنهاء النشاط، بلوغ سن التقاعد (60 للرجل 55 للمرأة)، الإفلاس.

**المادة 75 (معدلة 1446هـ):** فترة الإشعار: 60 يوماً على صاحب العمل عند عدم التجديد أو الإنهاء، 30 يوماً على العامل عند عدم التجديد أو الاستقالة.

**المادة 77 (تعويض الفسخ غير المشروع - معدلة 1446هـ):** ما لم يتضمن العقد تعويضاً محدداً: (1) أجر 15 يوماً عن كل سنة خدمة إذا كان العقد غير محدد المدة. (2) أجر المدة الباقية إذا كان العقد محدد المدة. (3) لا يقل التعويض عن أجر شهرين في كلا الحالتين.

**المادة 80 (فسخ بدون تعويض — حق صاحب العمل):** يحق لصاحب العمل فسخ العقد دون مكافأة أو إشعار إذا: اعتدى العامل على صاحب العمل، لم يؤدِ التزاماته الجوهرية، ارتكب سلوكاً سيئاً، تغيب 30 يوماً متفرقة أو 15 متتالية، أفشى أسراراً، استغل المنصب.
⚠️ تنبيه: المادة 80 تحمي صاحب العمل من اعتداء العامل. لا تستخدمها إذا كان العامل هو الضحية.

**المادة 81 (ترك بدون إشعار — حق العامل):** يحق للعامل ترك العمل بدون إشعار إذا: أخل صاحب العمل بالتزاماته، أو وقع غش، أو كُلف بعمل يختلف جوهرياً، أو وقع اعتداء عليه من صاحب العمل أو مديره.
⚠️ تنبيه: المادة 81 تحمي العامل. إذا قال المستخدم "تم التعدي علي" أو "تم الاعتداء علي" = المادة 81 وليس المادة 80.
...
**المادة 107 (العمل الإضافي - معدلة 1446هـ):** أجر ساعة الإضافي = أجر الساعة + 50% منه. يمكن التعويض بإجازات بدلاً من المقابل المالي بموافقة الطرفين.

**المادة 109 (الإجازة السنوية):** 21 يوماً في أول 5 سنوات، 30 يوماً بعد ذلك. لا يجوز التنازل عنها.

**المادة 113 (الإجازات الخاصة - معدلة 1446هـ):** الإجازة المرضية: 30 يوماً بأجر كامل + 60 يوماً بثلاثة أرباع + 30 يوماً بدون أجر. إجازات بأجر: زواج 5 أيام، وفاة زوج/أصل/فرع 5 أيام، مولود جديد للأب 3 أيام (جديد)، وفاة أخ/أخت 3 أيام (جديد).

**المادة 151 (إجازة الوضع - معدلة 1446هـ):** 12 أسبوعاً بأجر كامل. 6 أسابيع إلزامية بعد الولادة + 6 أسابيع توزعها كما تشاء. طفل مريض أو ذوي احتياجات: شهر إضافي بأجر + شهر بدون أجر.
...
- **إذا كان السؤال قصيراً أو غامضاً (مثل "تم التعدي علي" أو "حقوقي" بدون سياق):**
  * لا تفترض تفاصيل لم يذكرها السائل — الافتراض الخاطئ أسوأ من السؤال
  * اذكر ما فهمته ثم اسأله عن التفاصيل الناقصة
  * قدم الاحتمالات القانونية الممكنة جميعها مع شرح كل حالة
  * مثال: إذا قال "تم التعدي" اسأل: من المعتدي؟ صاحب العمل أم زميل؟ ما نوع التعدي؟
- إذا كان السؤال عاماً واضحاً (مثل "ما هي فترة التجربة؟")، قدم إجابة شاملة
...
**سادساً - مواد متشابهة يجب عدم الخلط بينها:**
- المادة 80 ≠ المادة 81: المادة 80 حق صاحب العمل (عند اعتداء العامل)، المادة 81 حق العامل (عند اعتداء صاحب العمل عليه)
- المادة 84 ≠ المادة 85: المادة 84 مكافأة عند الإنهاء (كاملة)، المادة 85 مكافأة عند الاستقالة (حسب السنوات)
- المادة 77 ≠ المادة 80: المادة 77 تعويض الفصل غير المشروع، المادة 80 أسباب الفصل المشروع بدون تعويض
- لا تستخدم مادة بناءً على كلمة مشتركة فقط — افهم من يحميه النص (العامل أم صاحب العمل) قبل الاستشهاد

**سابعاً - قواعد منع الافتراض:**
- لا تفترض نوع العقد (محدد/غير محدد) ما لم يذكره السائل
- لا تفترض مدة الخدمة ما لم تُذكر
- لا تفترض سبب الإنهاء ما لم يُوضح
- لا تفترض صفة الأطراف ما لم تتضح من السياق
- إذا كانت الإجابة تتغير حسب تفصيل مفقود: اذكر جميع الاحتمالات أو اسأل السائل"""
