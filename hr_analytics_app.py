...
LEGAL_TOPICS = {
    ...
    "insurance":       {"kw": ["تأمين","تأمينات","تأمينات اجتماعية","gosi","ساند","تقاعد","اشتراك","معاش"], "ref": "نظام التأمينات", "label": "التأمينات الاجتماعية",
        "facts": "سعودي: 9.75% خصم + 11.75% شركة. غير سعودي: 2% أخطار فقط. ساند: 0.75%+0.75%. التقاعد: سن 60 + 120 شهر أو 300 شهر مبكر. ساند: 60% لـ 3 أشهر ثم 50% لـ 9 أشهر أقصى 9000."},
...
def build_reasoning_context(question, advisor_type, analysis, retrieved_context=""):
    ...
    if advisor_type == "legal":
        ...
        if facts:
            ...
            header += "**التأمينات الاجتماعية (GOSI):** سعودي: معاشات 9%+9% + أخطار 2% + ساند 0.75%+0.75% = خصم العامل 9.75% + تحمل الشركة 11.75%. غير سعودي: أخطار 2% فقط على الشركة. ساند: 60% لأول 3 أشهر ثم 50% لـ9 أشهر (أقصى 9000). التقاعد: سن 60 + 120 شهر اشتراك أو 300 شهر مبكر\n"
...
def verify_and_correct_response(response, consultant_type="legal"):
    ...
    response = response.replace('خصم الموظف السعودي: 10.5%', 'خصم الموظف السعودي: 9.75%')
    response = response.replace('تحمّل الشركة: 12.5%', 'تحمّل الشركة: 11.75%')
...
GOSI: سعودي 9.75% موظف + 11.75% شركة | غير سعودي 2% شركة فقط
...
GOSI: السعودي: معاشات 9% موظف + 9% شركة + أخطار 2% شركة + ساند 0.75%+0.75% = إجمالي 9.75% موظف + 11.75% شركة. غير السعودي: 2% أخطار مهنية على الشركة فقط."""
