            ...
            **المصادر المعتمدة:**
            ...
            if '_pending_labor_q' in st.session_state:
                ...
                with st.spinner("جاري التحليل القانوني..."):
                    try:
                        # === INTENT DETECTOR: understand question BEFORE AI ===
                        q_lower = pending_q.lower()
                        intent_hint = ""
                        # Assault ON the worker → Art 81
                        if any(w in q_lower for w in ['تعدي علي','تعدى علي','اعتدى علي','ضربني','أهانني','اهانني','تم التعدي','تم الاعتداء','يضربني','يهينني','أساء لي','اساء لي','معاملة سيئة','قسوة','إهانة من','ظلم من']):
                            intent_hint = "★★★ هذا السؤال عن تعدي صاحب العمل على العامل. الجواب الصحيح: المادة 81 - يحق للعامل ترك العمل فوراً بدون إشعار مع الاحتفاظ بكامل حقوقه (مكافأة نهاية الخدمة + تعويض). لا تذكر المادة 80 لأنها عن تعدي العامل على صاحب العمل وليس العكس.\n\n"
                        # Assault BY the worker → Art 80
                        elif any(w in q_lower for w in ['موظف اعتدى','موظف تعدى','عامل ضرب','عامل اعتدى','تعدى على المدير','ضرب المدير','اعتدى على صاحب']):
                            intent_hint = "★★★ هذا السؤال عن تعدي العامل على صاحب العمل. الجواب: المادة 80 - يحق فصله بدون مكافأة أو تعويض أو إشعار.\n\n"
                        # Termination without cause → Art 77
                        elif any(w in q_lower for w in ['فصل تعسفي','فصلوني بدون سبب','طردوني','فُصلت','تم فصلي','إنهاء غير مشروع','فصل غير']):
                            intent_hint = "★★★ هذا سؤال عن فصل تعسفي. الجواب: المادة 77 - تعويض (إذا العقد فيه تعويض محدد يُطبق أولاً، وإلا: 15 يوم/سنة أو المدة الباقية، أدنى شهرين) + مكافأة نهاية الخدمة م84.\n\n"
                        # Resignation → Art 85
                        elif any(w in q_lower for w in ['أستقيل','استقالة','أبي أترك','أريد أترك','أريد الاستقالة','ناوي أستقيل','أبغى أستقيل']):
                            intent_hint = "★★★ هذا سؤال عن الاستقالة. الجواب: المادة 85 - المكافأة حسب سنوات الخدمة (<2=0، 2-5=ثلث، 5-10=ثلثين، >10=كاملة) + إشعار 30 يوم.\n\n"
                        # Absence → Art 80
                        elif any(w in q_lower for w in ['غياب','تغيب','انقطع','انقطاع','ما حضر']):
                            intent_hint = "★★★ الغياب: المادة 80 - الفصل يكون بعد غياب أكثر من 30 يوم متفرقة أو أكثر من 15 يوم متصلة في السنة. الإنذار الكتابي عند 20 متفرقة أو 10 متصلة.\n\n"
                        # Probation → Art 53
                        elif any(w in q_lower for w in ['تجربة','فترة التجربة','probation','تحت التجربة']):
                            intent_hint = "★★★ فترة التجربة: المادة 53 المعدلة - 180 يوم أقصى، التمديد محظور.\n\n"

                        sys_p = f"""{intent_hint}أنت مستشار قانوني سعودي متخصص في نظام العمل المعدّل 2025.

فرّق بين:
- تعدي على العامل (من صاحب العمل) = المادة 81: ترك العمل فوراً مع كل الحقوق
- تعدي من العامل (على صاحب العمل) = المادة 80: فصل بدون مكافأة
- فصل بدون سبب = المادة 77: تعويض
- استقالة = المادة 85: مكافأة حسب المدة
...
★ اللغة: أجب بنفس لغة السؤال. إذا السؤال بالإنجليزية أجب بالإنجليزية بالكامل. إذا بالعربية أجب بالعربية.
