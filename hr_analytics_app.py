...
def verify_and_correct_response(response, consultant_type="legal"):
    """Post-process AI response to catch and correct known errors."""
    ...
    corrections = {
        ...
        # Article 77 corrections - must mention conditional
        # Article 80 corrections
    ...
    # Check Article 77 mentions conditional
    if 'المادة 77' in response or 'مادة 77' in response:
        if 'ما لم يتضمن العقد' not in response and 'تضمّن العقد' not in response and 'تضمن العقد' not in response and 'يُعمل بالعقد' not in response:
            response += '\n\n**ملاحظة هامة بخصوص المادة 77:** التعويضات المذكورة أعلاه تُطبق فقط إذا لم يتضمن العقد شرط تعويض محدد مقابل الإنهاء. إذا تضمّن العقد تعويضاً محدداً فيُعمل بما في العقد (الأصل).'

    # Verify article numbers
    for a in re.findall(r'المادة\s*(\d+)', response):
        if int(a) > 245:
            response = response.replace(f'المادة {a}', f'[المادة {a} - رقم غير صحيح، يُرجع للنظام]')

    # GOSI rate corrections
    response = response.replace('خصم الموظف السعودي: 10.5%', 'خصم الموظف السعودي: 9.75% (قديم) أو 10.25% (جديد)')
    response = response.replace('تحمّل الشركة: 12.5%', 'تحمّل الشركة: 11.75% (قديم) أو 12.25% (جديد)')
    response = response.replace('9.75% على الموظف', '9% على الموظف')
    response = response.replace('المعاشات: 9.75%', 'المعاشات: 9%')
...
أجب بالعربية. اتبع الخطوات السبع. الدقة أولاً."""
            ...
            **المصادر المعتمدة:**
            ...
            if submitted and labor_q and labor_q.strip():
                with st.spinner("جاري التحليل القانوني المعمّق..."):
                    ...
                    # === GROUNDED PROMPT: Inject actual law text so AI answers from source ===
                    LAW_REFERENCE = """[مرجع نظام العمل السعودي المعدّل - المرسوم الملكي م/44 نافذ 19/2/2025]

م53 التجربة: أقصاها 180 يوماً من بداية العقد. التمديد محظور. لا تدخل فيها إجازات الأعياد والمرضية. لكلا الطرفين الإنهاء بدون تعويض أو مكافأة. لا تُكرر إلا في مهنة مختلفة.
م55 تحول العقد: يتحول المحدد لغير محدد بعد 3 تجديدات أو 4 سنوات أيهما أقل، أو باستمرار العمل بعد الانتهاء.
م74 انتهاء العقد: باتفاق الطرفين (بموافقة كتابية)، انتهاء المدة، إرادة أحد الطرفين في غير المحدد، القوة القاهرة، إغلاق المنشأة نهائياً، إلغاء النشاط، بلوغ سن التقاعد (60 رجال/55 نساء ما لم يُتفق على الاستمرار)، وفاة العامل أو عجزه الكلي.
م75 الإشعار: لإنهاء العقد غير المحدد يجب إشعار مكتوب قبل 60 يوماً (الأجر الشهري) أو 30 يوماً (غيره). فترة الإشعار يستمر العقد.
م76 تعويض الإشعار: من لا يلتزم بمهلة الإشعار يدفع تعويضاً يعادل أجر المهلة.
م77 التعويض عن الإنهاء غير المشروع: ★★★ إذا تضمّن العقد تعويضاً محدداً مقابل إنهائه يُعمل به (الأصل). إذا لم يتضمن العقد تعويضاً: غير محدد المدة = 15 يوم أجر عن كل سنة خدمة. محدد المدة = أجر المدة الباقية. الحد الأدنى في الحالتين = أجر شهرين.
م78 يوم الفصل: يعتبر انتهاء العلاقة من تاريخ إبلاغ الطرف الآخر.
م80 فصل بدون مكافأة: يحق لصاحب العمل فسخ العقد بدون مكافأة أو إشعار أو تعويض في الحالات: اعتداء على صاحب العمل أو المدير، عدم أداء الالتزامات الجوهرية أو عدم إطاعة الأوامر المشروعة بعد إنذار كتابي، ثبوت اتباع سلوك سيئ أو عمل مخل بالشرف، إلحاق خسارة مادية عمداً (يُبلغ خلال 24 ساعة)، التزوير للحصول على العمل، التعيين تحت التجربة، الغياب أكثر من 30 يوماً متفرقة أو أكثر من 15 يوماً متصلة في السنة (بعد إنذار كتابي عند 20 يوم متفرقة أو 10 أيام متصلة)، استغلال المنصب للكسب الشخصي، إفشاء أسرار العمل.
م81 ترك العمل بدون إشعار: يحق للعامل ترك العمل دون إشعار مع حفظ حقوقه إذا: لم يفِ صاحب العمل بالتزاماته التعاقدية أو النظامية الجوهرية، ثبت غش في شروط العمل، كُلّف بعمل مختلف جوهرياً بدون موافقته، وجود خطر جسيم يهدد سلامته، تعامل صاحب العمل أو مديره بقسوة أو إهانة، عدم معالجة ظلم واقع عليه.
م84 مكافأة نهاية الخدمة: أجر نصف شهر عن كل سنة من السنوات الخمس الأولى + أجر شهر كامل عن كل سنة بعد ذلك. الأجزاء تُحسب نسبياً. الأجر الأخير هو أساس الحساب.
م85 المكافأة عند الاستقالة: أقل من سنتين = لا شيء. من 2 إلى 5 سنوات = ثلث المكافأة. من 5 إلى 10 = ثلثا المكافأة. أكثر من 10 = المكافأة كاملة. المرأة تستحق المكافأة كاملة إذا أنهت العقد خلال 6 أشهر من الزواج أو 3 أشهر من الوضع.
م88 التسوية: يلتزم صاحب العمل بدفع أجر العامل وتصفية حقوقه خلال أسبوع من انتهاء العلاقة. إذا أنهى العامل العقد يُسوى خلال أسبوعين.
م98 ساعات العمل: 8 ساعات/يوم أو 48 ساعة/أسبوع. في رمضان: 6 ساعات/يوم أو 36 ساعة/أسبوع.
م107 الأجر الإضافي: يجوز تعويض ساعات العمل الإضافية بإجازة مدفوعة بدلاً من الأجر الإضافي.
م109 الإجازة السنوية: 21 يوماً. تزداد إلى 30 يوماً بعد 5 سنوات خدمة متصلة. لا يجوز التنازل عنها أو التعويض عنها نقداً أثناء الخدمة.
م113 إجازة أبوة: 3 أيام مدفوعة.
م151 إجازة الوضع (المعدلة): 12 أسبوعاً بأجر كامل. 6 أسابيع إلزامية بعد الولادة. الباقي توزعه حسب رغبتها (قبل أو بعد). يُحظر تشغيلها خلال 6 الأسابيع التالية للولادة. إذا أنجبت طفلاً مريضاً أو ذا إعاقة: شهر إضافي مدفوع قابل للتمديد بدون أجر.
فترة الإشعار: 60 يوماً من صاحب العمل، 30 يوماً من العامل.
الاستقالة (التعديل): تعريف جديد - إعلان كتابي من العامل برغبته في إنهاء العقد.
GOSI النظام القديم (قبل 3/7/2024): معاشات 9%+9% + أخطار 2% + ساند 0.75%+0.75% = موظف 9.75% + شركة 11.75%. النظام الجديد (بعد 3/7/2024): معاشات تزداد تدريجياً من 9% إلى 11% (حالياً 9.5%+9.5%) = موظف 10.25% + شركة 12.25%. التقاعد الجديد: 65 سنة. غير السعودي: 2% شركة فقط."""

                    cot_prompt = f"""أنت مستشار قانوني خبير في نظام العمل السعودي.

{LAW_REFERENCE}

{f'معلومات إضافية من قاعدة المعرفة:{chr(10)}{rag_ctx[:2000]}' if rag_ctx else ''}

★★★ تعليمات الإجابة:
1. أجب فقط من المرجع أعلاه. لا تخترع نصوصاً غير موجودة فيه.
2. إذا سُئلت عن مادة غير موجودة في المرجع اكتب "هذه المادة غير متوفرة في المرجع الحالي، يُرجع للنص الرسمي لنظام العمل".
3. كل مادة لها شروط واستثناءات. اذكرها كلها.
4. المادة 77 مهمة: التعويض المحدد في العقد يُقدّم على التعويض النظامي. اذكر هذا دائماً.
5. أجب بتفصيل وعمق مع حسابات رقمية إذا لزم.
6. نسّق الإجابة بعناوين وأرقام وأمثلة عملية.
7. في النهاية اكتب: "ملاحظة: هذه استشارة أولية. يُنصح بالتحقق من النص الرسمي ومراجعة محامٍ متخصص للحالات المعقدة."
"""
                    user_msg = f"السؤال: {labor_q}"
